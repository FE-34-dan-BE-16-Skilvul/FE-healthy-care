...
const Cart = () => {
  ...
  const handleCloseCart = (close) => {
    ...
    if (close && cartQuantity > 0) {
      const cartCalories = cartItems.reduce(
        (totalCalories, item) => totalCalories + item.kalori * item.quantity,
        0
      );
      localStorage.setItem("cartCalories", cartCalories);
      toast.success("Data kalori berhasil disimpan!");
    }

    // Reload page ketika keranjang ditutup
    if (!close) {
